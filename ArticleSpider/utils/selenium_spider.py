...
def oschina_blog_test():
    browser = webdriver.Chrome(executable_path='/opt/chromedriver')
    browser.get("https://www.oschina.net/blog")

    tab_bar = browser.find_element_by_css_selector(".box .blog-tabs")
    action = webdriver.common.action_chains.ActionChains(browser)
    action.move_to_element_with_offset(tab_bar,200,20)
    action.click()
    action.perform()
    time.sleep(10)
    # for i in range(3):
...
def taobao_detail_test():
    browser = webdriver.PhantomJS(executable_path='/opt/phantomjs')
